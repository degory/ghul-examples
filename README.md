...
The project is configured to use the standard .NET 10 SDK image, [`mcr.microsoft.com/dotnet/sdk:10.0`](https://hub.docker.com/r/microsoft/dotnet-sdk). It carries the SDK, git and curl and little else, which keeps the first start of a Codespace as short as possible. You can use a different container image if you prefer - just edit the configuration in the `.devcontainer` folder accordingly. Make sure the image you select includes the .NET 10 SDK, with `dotnet` on the PATH.
